...
const ModalEdit = ({ id, show, close, stateModal, setStateModal }) => {
  const appContextModal = useContext(appContext)
  ...
  function updateRegister (e) {
    axios.put(`http://localhost:3001/updateRegister/${id}`, stateModal).then((res) => {
      ...
      if (res.status === 200) {
        ...
        appContextModal.fetchTareas(appContextModal.fecha)
      } else {
  ...
  return (
    <>
      ...
      <Modal show={show} onHide={close}>
        <Modal.Header closeButton className='modal-hea-foo modal-color'>
          <Modal.Title>Actualiza tu tarea</Modal.Title>
        </Modal.Header>
        <Modal.Body className='modal-body modal-color'>
          <Form.Group>
            <Form.Group className='mb-3' controlId='exampleForm.ControlInput1'>
              <Form.Label>Titulo</Form.Label>
              <Form.Control
                type='text' placeholder='Titulo Nuevo' autoFocus name='titulo' onChange={saveFormModal}
            ...
            >
              ...
              <Form.Control
                as='textarea' rows={3} name='descripcion' onChange={saveFormModal}
        ...
        <Modal.Footer className='modal-hea-foo'>
